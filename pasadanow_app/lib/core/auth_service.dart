...
// ── AuthService ────────────────────────────────────────────────────────────
class AuthService {
  // ✅ FIX: baseUrl is now just the host — paths include /api/auth/
  //         to match Spring Boot's actual route mappings.
  final Dio _dio = Dio(BaseOptions(
    baseUrl: ApiConstants.springBase, // e.g. http://localhost:8080
  ...
  Future<Map<String, dynamic>> login(String username, String password) async {
    try {
      // ✅ FIX: /api/auth/login — matches Spring Boot SecurityConfig
  ...
  Future<void> register(Map<String, dynamic> data) async {
    ...
    try {
      // ✅ FIX: /api/auth/register — matches Spring Boot SecurityConfig
  ...
  Future<void> logout() async {
    try {
      // ✅ FIX: /api/auth/logout — matches Spring Boot SecurityConfig
