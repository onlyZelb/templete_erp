...
class AuthService {
  ...
  Future<Map<String, dynamic>> login(String username, String password) async {
    try {
      ...
      try {
        ...
        await _storage.write(key: _keyVerifiedStatus, value: verifiedStatus ?? '');
        await _storage.write(key: _keyUsername,       value: returnedUser ?? username);
      } catch (_) {}
  ...
  Future<Map<String, dynamic>> googleLogin() async {
    ...
    if (tokenToSend == null) {
      throw const AuthException('Could not get Google token. Please try again.');
    }
    ...
    try {
      ...
      try {
        ...
        await _storage.write(key: _keyRole,           value: role);
        await _storage.write(key: _keyVerifiedStatus, value: verifiedStatus ?? '');
        await _storage.write(key: _keyUsername,       value: returnedUser ?? googleUser.email);
      } catch (_) {}
  ...
  Future<void> logout({String? role}) async {
    try {
      ...
      try {
        ...
        if (rmEnabled  != null) await _storage.write(key: _rmEnabled,  value: rmEnabled);
        if (rmUsername != null) await _storage.write(key: _rmUsername, value: rmUsername);
        if (rmPassword != null) await _storage.write(key: _rmPassword, value: rmPassword);
      } catch (_) {}
    }
  }
  ...
  Future<String?> getToken() async {
    try { return await _storage.read(key: _keyToken); } catch (_) { return null; }
  }

  Future<String?> getRole() async {
    try { return await _storage.read(key: _keyRole); } catch (_) { return null; }
  }

  Future<String?> getVerifiedStatus() async {
    try { return await _storage.read(key: _keyVerifiedStatus); } catch (_) { return null; }
  }

  Future<String?> getUsername() async {
    try { return await _storage.read(key: _keyUsername); } catch (_) { return null; }
  }
