...
class CommuterProfile {
  ...
  factory CommuterProfile.fromJson(Map<String, dynamic> json) {
    return CommuterProfile(
      ...
      profilePhotoUrl: json['profile_photo']?.toString() ?? json['profilePhoto']?.toString(),
    );
  }

  CommuterProfile copyWith({
    String? fullName, String? age, String? phone,
    String? email, String? address, String? profilePhotoUrl,
  }) => CommuterProfile(
    username:        username,
...
class _CommuterHomeState extends State<CommuterHome>
  ...
  void initState() {
    ...
    _profileRefreshTimer =
        Timer.periodic(const Duration(seconds: 30), (_) => _loadCommuterProfile());
  }
  ...
  Future<void> _initGPS() async {
    ...
    try {
      ...
      if (last != null && mounted) {
        ...
        setState(() {
          _myLocation = loc; _gpsAccuracy = last.accuracy;
          _gpsSpeed = last.speed; _gpsHeading = last.heading;
          _lastGpsUpdate = DateTime.now(); _isTracking = true;
        });
    ...
    try {
      ...
      if (mounted) {
        ...
        setState(() {
          _myLocation = loc; _gpsAccuracy = pos.accuracy;
          _gpsSpeed = pos.speed; _gpsHeading = pos.heading;
          _lastGpsUpdate = DateTime.now(); _isTracking = true; _gpsForced = false;
        });
        _mapController.move(loc, 16);
      }
    } catch (_) {}
    ...
    _locationStream = Geolocator.getPositionStream(
      ...
      (Position pos) {
        ...
        setState(() {
          _myLocation = updated; _gpsAccuracy = pos.accuracy;
          _gpsSpeed = pos.speed; _gpsHeading = pos.heading;
          _lastGpsUpdate = DateTime.now(); _isTracking = true;
        });
  ...
  Future<void> _pushCommuterLocation(LatLng loc) async {
    ...
    try {
      ...
      await dio.post('/rides/${_activeBooking!['id']}/location', data: {
        ...
        'accuracy': _gpsAccuracy, 'speed': _gpsSpeed, 'heading': _gpsHeading,
      });
    } catch (_) {}
  }
  ...
  void _useFallback() {
    ...
    if (mounted) {
      setState(() { _myLocation = fallback; _isTracking = false; });
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Row(children: [
          ...
          const Text('GPS is Off', style: TextStyle(color: _textPrim, fontSize: 16)),
        ]),
        content: const Text(
        ...
        actions: [
          TextButton(
            onPressed: () { Navigator.pop(context); setState(() => _gpsForced = false); },
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Row(children: [
          ...
          const SizedBox(width: 10),
          const Text('Permission Denied', style: TextStyle(color: _textPrim, fontSize: 16)),
        ]),
        content: const Text(
        ...
        actions: [
          ...
          ElevatedButton.icon(
            ...
            onPressed: () async { Navigator.pop(context); await Geolocator.openAppSettings(); },
          ),
        ],
      ),
    );
  ...
  Future<void> _fetchRoute() async {
    ...
    setState(() { _routeLoading = true; _routePoints = []; });

    try {
      final a = await _geocode(_pickup.text.trim());
      final b = await _geocode(_destination.text.trim());
      if (a == null || b == null) {
        if (mounted) _showSnack('Could not find one of the locations.', _orange);
        setState(() => _routeLoading = false);
        return;
      }
      setState(() { _pickupLatLng = a; _destLatLng = b; });
      ...
      final res = await http.get(osrmUri, headers: {'Accept': 'application/json'});
      ...
      setState(() => _routeLoading = false);
      if (mounted) _showSnack('Route fetch failed. Check connection.', Colors.redAccent);
    }
  }
    ...
    try {
      ...
      if (mounted) {
        final fetched = List<Map<String, dynamic>>.from(
          (res.data as List).map((d) {
            ...
            return {
              ...
              'initials': initials,
              'name':     name,
              'plate':    '${d['plate_number'] ?? d['plate'] ?? '—'} · ${d['vehicle_type'] ?? 'Vehicle'}',
        ...
        setState(() {
          ...
          if (!stillOnline) { _selectedDriverId = null; _selectedDriver = null; }
        });
      }
    } catch (_) {
    ...
    try {
      ...
      setState(() {
        _activeBooking = rideData is Map
            ? Map<String, dynamic>.from(rideData) : null;
      ...
      _destination.clear();
      setState(() {
        _fare = null; _selectedDriver = null; _selectedDriverId = null;
        _routePoints = []; _pickupLatLng = null; _destLatLng = null;
        _routeDistKm = null; _routeDurationMin = null;
      });
      ...
      if (mounted) _showSnack('Booking failed. Please try again.', Colors.redAccent);
    } finally {
  ...
  Future<void> _pollRideStatus() async {
    if (_activeBooking == null) { _rideStatusTimer?.cancel(); return; }
    try {
        ...
        if (prevStatus != newStatus) {
          if (newStatus == 'accepted' || newStatus == 'ongoing') {
            ...
            if (_myLocation != null && _driverLiveLocation != null) {
              final bounds = LatLngBounds.fromPoints([_myLocation!, _driverLiveLocation!]);
              _mapController.fitCamera(CameraFit.bounds(
            ...
            await _loadRides();
            setState(() {
              _activeBooking = null; _activeBookingStatus = null; _driverLiveLocation = null;
            });
          ...
          Container(
            ...
            child: ClipRRect(
              ...
              child: Image.asset(
                ...
                errorBuilder: (_, __, ___) => Icon(_IC.tricycle, color: _accent, size: 22),
              ),
            ),
          ),
          const SizedBox(width: 12),
          // Brand name + greeting column
          Column(
  ...
  Widget _buildAvatarWidget({required double radius, required double fontSize}) {
    final photoUrl = _profile?.profilePhotoUrl;
    ...
    return CircleAvatar(
      ...
      backgroundColor: _o(_accent, 0.2),
      child: Text(initial,
          style: TextStyle(color: _accent, fontSize: fontSize, fontWeight: FontWeight.bold)),
    );
  }
  ...
  Widget _buildBottomNav() => Container(
        ...
        child: BottomNavigationBar(
          ...
          items: [
            BottomNavigationBarItem(icon: Icon(_IC.dashboard), label: 'Overview'),
            BottomNavigationBarItem(
                ...
                label: 'Trip Records'),
            BottomNavigationBarItem(icon: Icon(_IC.profile), label: 'My Profile'),
          ],
        ),
      );
  ...
  Widget _buildDashboard(AuthProvider auth) {
    return SingleChildScrollView(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        // Driver status + GPS badge row (replaces the old greeting row)
        Padding(
  ...
  Widget _gpsInfoBadge() => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            const Text('GPS LIVE',
                style: TextStyle(color: _green, fontSize: 8, fontWeight: FontWeight.w700)),
          ]),
          Text(_gpsSpeedLabel,
              style: const TextStyle(color: _textPrim, fontSize: 11, fontWeight: FontWeight.w700)),
          Text(_gpsAccuracyLabel, style: TextStyle(color: _gpsAccuracyColor, fontSize: 8)),
        ]),
      );
  ...
  Widget _buildActiveRideBanner() {
    ...
    switch (status) {
      case 'accepted':
      case 'ongoing':
        statusColor = _green; statusText = 'Driver on the way!'; statusIcon = _IC.tricycle; break;
      case 'completed':
        statusColor = _accent; statusText = 'Ride completed'; statusIcon = _IC.check; break;
      default:
        statusColor = _orange; statusText = 'Waiting for driver…'; statusIcon = Icons.hourglass_top_rounded;
    }
    return Container(
      margin: const EdgeInsets.fromLTRB(16, 12, 16, 0),
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
      decoration: BoxDecoration(
        color: _o(statusColor, 0.1), borderRadius: BorderRadius.circular(12),
        ...
        if (status == 'pending')
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: _o(_red, 0.1), borderRadius: BorderRadius.circular(8),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Text('Active Ride',
              style: TextStyle(color: _textPrim, fontSize: 14, fontWeight: FontWeight.w700)),
        ]),
  ...
  Widget _detailRow(IconData icon, String label, String value) => Row(
        ...
        children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(value, style: const TextStyle(color: _textPrim, fontSize: 13, fontWeight: FontWeight.w600)),
          ]),
        ],
      );
    ...
    return Row(
      children: List.generate(steps.length * 2 - 1, (i) {
        if (i.isOdd) {
          ...
          return Expanded(child: Container(height: 2, color: filled ? _green : _cardBorder));
        }
        final stepIdx = i ~/ 2;
        ...
        return Column(children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: done ? _green : _o(_textSub, 0.3), width: 2),
            ),
          ),
      ...
      Expanded(
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8),
            Text(value,
                style: const TextStyle(color: _textPrim, fontSize: 13, fontWeight: FontWeight.w800),
                maxLines: 1, overflow: TextOverflow.ellipsis),
            const SizedBox(height: 2),
            Text(label,
                style: const TextStyle(color: _textSub, fontSize: 7.5, fontWeight: FontWeight.w600),
                maxLines: 2),
          ]),
        ),
      );
            ...
            FlutterMap(
              ...
              children: [
                ...
                if (_myLocation != null && _gpsAccuracy != null && _isTracking)
                  CircleLayer(circles: [
                    CircleMarker(
                      point: _myLocation!, radius: _gpsAccuracy!,
                      useRadiusInMeter: true,
                      color: _o(_green, 0.07), borderColor: _o(_green, 0.2),
                      borderStrokeWidth: 1.0,
                    ),
                  ]),
                if (_routePoints.isNotEmpty)
                  PolylineLayer<Object>(polylines: [
                    Polyline(points: _routePoints, strokeWidth: 7.0, color: _o(Colors.black, 0.25)),
                    Polyline(points: _routePoints, strokeWidth: 4.5, color: _routeColor),
                  ]),
                MarkerLayer(markers: [
                  if (_myLocation != null)
                    Marker(
                      point: _myLocation!, width: 140, height: 64,
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        Container(
                          width: 14, height: 14,
                          decoration: BoxDecoration(
                            color: _isTracking ? _green : _orange, shape: BoxShape.circle,
                  ...
                  if (_driverLiveLocation != null)
                    Marker(
                      point: _driverLiveLocation!, width: 120, height: 64,
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                          decoration: BoxDecoration(
                  ...
                  if (_pickupLatLng != null)
                    Marker(
                      point: _pickupLatLng!, width: 110, height: 56,
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  ...
                  if (_destLatLng != null)
                    Marker(
                      point: _destLatLng!, width: 120, height: 56,
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            ...
            if (_isTracking && _gpsAccuracy != null)
              Positioned(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _o(_navyLight, 0.9), borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: _cardBorder),
                  ),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Text(_gpsSpeedLabel,
                        style: const TextStyle(color: _textPrim, fontSize: 10, fontWeight: FontWeight.w700)),
                  ]),
                ),
              ),
            if (!_isTracking)
              Positioned(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _o(_orange, 0.93), borderRadius: BorderRadius.circular(10),
            ...
            if (_routeLoading)
              Container(
                ...
                child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(height: 8),
                  const Text('Calculating route…', style: TextStyle(color: Colors.white, fontSize: 11)),
                ])),
              ),
            Positioned(
              right: 6, bottom: 4,
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: _o(Colors.white, 0.75), borderRadius: BorderRadius.circular(3),
                ),
  ...
  Widget _mapBtn(IconData icon, VoidCallback onTap) => GestureDetector(
        ...
        child: Container(
          width: 32, height: 32,
          decoration: BoxDecoration(
            color: _o(_navyLight, 0.92), borderRadius: BorderRadius.circular(8),
            border: Border.all(color: _cardBorder),
            boxShadow: const [BoxShadow(color: Colors.black26, blurRadius: 4)],
          ),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const Text('Route & Fare Details',
                style: TextStyle(color: _textPrim, fontSize: 13, fontWeight: FontWeight.w700)),
          ]),
          const SizedBox(height: 12),
          Row(children: [
            _metricChip(_IC.distance, '${_routeDistKm!.toStringAsFixed(2)} km',        'Distance', _accent),
            const SizedBox(width: 8),
            _metricChip(_IC.time,     '${_routeDurationMin!.toStringAsFixed(0)} min',   'Est. Time', _purple),
            const SizedBox(width: 8),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: _o(_green, 0.06), borderRadius: BorderRadius.circular(8),
            ...
            child: Column(children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                const Text('TOTAL FARE',
                    style: TextStyle(color: _textPrim, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 0.5)),
                Text(_computedFare,
                    style: const TextStyle(color: _green, fontSize: 14, fontWeight: FontWeight.w800)),
              ]),
            ]),
          ),
        ]),
  ...
  Widget _metricChip(IconData icon, String value, String label, Color color) =>
      Expanded(
        child: Container(
          ...
          decoration: BoxDecoration(
            color: _o(color, 0.1), borderRadius: BorderRadius.circular(10),
          ...
          child: Column(children: [
            ...
            Text(label, style: const TextStyle(color: _textSub, fontSize: 9, fontWeight: FontWeight.w600)),
          ]),
        ),
      );
  ...
  Widget _fareBreakdownRow(String label, String amount) => Padding(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Text(amount, style: const TextStyle(color: _textPrim, fontSize: 11, fontWeight: FontWeight.w600)),
        ]),
      );
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(width: 8, height: 8, decoration: const BoxDecoration(color: _accent, shape: BoxShape.circle)),
          const SizedBox(width: 8),
          const Text('Book a Ride', style: TextStyle(color: _textPrim, fontSize: 14, fontWeight: FontWeight.w700)),
          const Spacer(),
          if (_routeLoading)
            Row(children: [
              ...
              const SizedBox(width: 6),
              const Text('Finding route…', style: TextStyle(color: _accent, fontSize: 10)),
            ])
        ...
        if (_isTracking && _myLocation != null && _pickup.text.isEmpty)
          GestureDetector(
            onTap: () {
              setState(() { _pickup.text = 'My current location'; _pickupLatLng = _myLocation; });
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: _o(_green, 0.08), borderRadius: BorderRadius.circular(8),
        ...
        const SizedBox(height: 6),
        _inputField(_destination, 'Enter destination...',    _IC.destination, _orange),
        const SizedBox(height: 4),
        const Text('Route & fare auto-calculates as you type',
            style: TextStyle(color: _textSub, fontSize: 10, fontStyle: FontStyle.italic)),
        const SizedBox(height: 12),
  ...
  Widget _fieldLabel(String text) => Text(text,
      style: const TextStyle(color: _textSub, fontSize: 10, fontWeight: FontWeight.w600, letterSpacing: 0.8));

  Widget _inputField(TextEditingController ctrl, String hint, IconData icon, Color accentColor) {
    return TextField(
      controller: ctrl,
  ...
  Widget _driverDropdown() {
    if (_driversLoading && _nearbyDrivers.isEmpty) {
      return Container(
        ...
        decoration: BoxDecoration(color: _navyLight, borderRadius: BorderRadius.circular(8), border: Border.all(color: _cardBorder)),
        child: Row(children: [
          const SizedBox(width: 14, height: 14, child: CircularProgressIndicator(color: _accent, strokeWidth: 2)),
          const SizedBox(width: 10),
          const Text('Loading online drivers…', style: TextStyle(color: _textSub, fontSize: 12)),
        ]),
      );
    }
    if (_nearbyDrivers.isEmpty) {
      return Container(
        ...
        decoration: BoxDecoration(color: _navyLight, borderRadius: BorderRadius.circular(8), border: Border.all(color: _cardBorder)),
        child: Row(children: [
          Icon(_IC.driver, color: _textSub, size: 18),
          const SizedBox(width: 8),
          const Expanded(child: Text('No drivers online — check back soon', style: TextStyle(color: _textSub, fontSize: 12))),
          GestureDetector(
            onTap: _loadNearbyDrivers,
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
              decoration: BoxDecoration(color: _o(_accent, 0.15), borderRadius: BorderRadius.circular(6), border: Border.all(color: _o(_accent, 0.3))),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(_IC.refresh, color: _accent, size: 12),
                const SizedBox(width: 4),
                const Text('Refresh', style: TextStyle(color: _accent, fontSize: 10)),
              ]),
            ),
          ),
        ]),
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          items: _nearbyDrivers.map((d) => DropdownMenuItem<String>(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Text(d['name'] as String, style: const TextStyle(color: _textPrim, fontSize: 12)),
                  const SizedBox(width: 6),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 1),
                    decoration: BoxDecoration(color: _o(_green, 0.12), borderRadius: BorderRadius.circular(4)),
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      Container(width: 5, height: 5, decoration: const BoxDecoration(color: _green, shape: BoxShape.circle)),
                      const SizedBox(width: 3),
                      const Text('Online', style: TextStyle(color: _green, fontSize: 8)),
                    ]),
                  ),
                ]),
                Text(d['plate'] as String, style: const TextStyle(color: _textSub, fontSize: 10)),
              ])),
            ]),
          )).toList(),
          onChanged: (v) {
            final driver = _nearbyDrivers.firstWhere((d) => d['id'] == v, orElse: () => {});
            setState(() { _selectedDriverId = v; _selectedDriver = driver['name'] as String?; });
          },
        ),
      ),
    );
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          const Text('Nearest Drivers', style: TextStyle(color: _textPrim, fontSize: 14, fontWeight: FontWeight.w700)),
          const Spacer(),
          GestureDetector(onTap: _loadNearbyDrivers, child: Icon(_IC.refresh, color: _textSub, size: 18)),
        ]),
        const SizedBox(height: 14),
        if (_driversLoading && _nearbyDrivers.isEmpty)
          const Padding(
          ...
          Padding(
            ...
            child: Center(child: Column(children: [
              ...
              const SizedBox(height: 8),
              const Text('No drivers online right now', style: TextStyle(color: _textSub, fontSize: 13)),
            ])),
          )
        else
        ...
        const Text('FLEET SUMMARY',
            style: TextStyle(color: _textSub, fontSize: 10, fontWeight: FontWeight.w600, letterSpacing: 0.8)),
        const SizedBox(height: 10),
        ...
        if (_isTracking && _gpsAccuracy != null) ...[
          ...
          _fleetRow('GPS Accuracy', '±${_gpsAccuracy!.toStringAsFixed(0)} m ($_gpsAccuracyLabel)', _gpsAccuracyColor),
          _fleetRow('Current Speed', _gpsSpeedLabel, _accent),
        ],
        if (_routeDistKm != null) ...[
          const Divider(color: _cardBorder, height: 20),
          _fleetRow('Route Distance', '${_routeDistKm!.toStringAsFixed(2)} km', _accent),
          _fleetRow('Est. Travel Time', '${_routeDurationMin!.toStringAsFixed(0)} min', _purple),
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(d['name'] as String,
                style: const TextStyle(color: _textPrim, fontSize: 13, fontWeight: FontWeight.w600)),
            Text(d['plate'] as String, style: const TextStyle(color: _textSub, fontSize: 11)),
          ])),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            decoration: BoxDecoration(
              color: _o(_green, 0.12), borderRadius: BorderRadius.circular(20),
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(width: 4),
              const Text('Online', style: TextStyle(color: _green, fontSize: 10, fontWeight: FontWeight.w600)),
            ]),
          ),
        ]),
      );
  ...
  Widget _driverAvatar(String initials) => CircleAvatar(
        ...
        child: Text(initials, style: const TextStyle(color: _accent, fontSize: 10, fontWeight: FontWeight.bold)),
      );
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Trip Records',
              style: TextStyle(color: _textPrim, fontSize: 20, fontWeight: FontWeight.w800)),
          if (_activeBooking != null) ...[
            const SizedBox(height: 8),
      ...
      Expanded(
        child: _rides.isEmpty
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                const SizedBox(height: 12),
                const Text('No trips yet', style: TextStyle(color: _textSub, fontSize: 15)),
                const SizedBox(height: 6),
  ...
  Widget _historyCard(dynamic r) {
    ...
    final statusColor = isActive ? _green : status == 'completed' ? _accent : _textSub;
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          width: 40, height: 40,
          decoration: BoxDecoration(color: _o(_accent, 0.1), borderRadius: BorderRadius.circular(10)),
          child: Center(child: Icon(_IC.tricycle, color: _accent, size: 20)),
        ),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('${r['pickup_location']} → ${r['destination']}',
              style: const TextStyle(color: _textPrim, fontSize: 12, fontWeight: FontWeight.w600),
              maxLines: 1, overflow: TextOverflow.ellipsis),
          const SizedBox(height: 4),
          Row(children: [
            Text('₱${r['fare']}',
            ...
            if (distKm != null && distKm != 'null') ...[
              const SizedBox(width: 6),
              Text('$distKm km', style: const TextStyle(color: _textSub, fontSize: 10)),
            ],
            const SizedBox(width: 8),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
              decoration: BoxDecoration(color: _o(statusColor, 0.1), borderRadius: BorderRadius.circular(20)),
              child: Text(status, style: TextStyle(color: statusColor, fontSize: 9, fontWeight: FontWeight.w600)),
            ),
            if (isActive) ...[const SizedBox(width: 4), _PulseDot(color: _green)],
          ]),
        ])),
            ...
            child: Column(children: [
                  ...
                  Positioned(
                    ...
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: const Icon(_IC.camera, color: Colors.white, size: 14),
                      ),
                    ),
                  ),
                ],
              ...
              Text(
                profile?.fullName.isNotEmpty == true
                    ? profile!.fullName
                    : auth.username ?? 'Commuter',
                style: const TextStyle(color: _textPrim, fontSize: 20, fontWeight: FontWeight.w800),
              ),
              const SizedBox(height: 2),
              Text('@${auth.username ?? ''}',
                  style: const TextStyle(color: _textSub, fontSize: 12)),
              ...
              if (profile?.phone.isNotEmpty == true)
                Text(profile!.phone, style: const TextStyle(color: _textSub, fontSize: 12)),
              const SizedBox(height: 10),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 5),
                decoration: BoxDecoration(
                  color: _o(_accent, 0.1), borderRadius: BorderRadius.circular(20),
            ...
            Padding(
              ...
              child: Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  _profileInfoRow(_IC.gpsOn, 'Broadcast',
                      _activeBooking != null ? 'Broadcasting' : 'On ride only'),
                ]),
              ),
            ),
            ...
            child: _buildEditableSection(
              ...
              onEdit: () => _showEditDialog(
                ...
                fields: [
                  ...
                  _EditField('Age', 'age', profile?.age ?? '', keyboardType: TextInputType.number),
                  _EditField('Phone Number', 'phone', profile?.phone ?? '', keyboardType: TextInputType.phone),
                  _EditField('Email Address', 'email', profile?.email ?? '', keyboardType: TextInputType.emailAddress),
                  _EditField('Address', 'address', profile?.address ?? ''),
                ],
              ),
              child: Column(children: [
                _profileInfoRow(_IC.badge,    'Full Name',    profile?.fullName.isNotEmpty  == true ? profile!.fullName  : '—'),
                _profileInfoRow(_IC.age,      'Age',          profile?.age.isNotEmpty       == true ? profile!.age       : '—'),
                _profileInfoRow(_IC.phone,    'Phone Number', profile?.phone.isNotEmpty     == true ? profile!.phone     : '—'),
                _profileInfoRow(_IC.email,    'Email Address',profile?.email.isNotEmpty     == true ? profile!.email     : '—'),
                _profileInfoRow(_IC.location, 'Address', profile?.address.isNotEmpty == true ? profile!.address : '—'),
              ]),
            ),
          ),
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 12),
                _profileInfoRow(_IC.person, 'Username',  auth.username ?? '—'),
          ...
          Padding(
            ...
            child: GestureDetector(
              ...
              child: Container(
                padding: const EdgeInsets.all(14),
                decoration: BoxDecoration(
                  color: _o(_purple, 0.06), borderRadius: BorderRadius.circular(12),
                  ...
                  const Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Change Password',
                          style: TextStyle(color: _textPrim, fontSize: 13, fontWeight: FontWeight.w600)),
                      SizedBox(height: 2),
          ...
          Padding(
            ...
            child: GestureDetector(
              ...
              child: Container(
                padding: const EdgeInsets.symmetric(vertical: 14),
                decoration: BoxDecoration(
                  color: _o(_red, 0.08), borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: _o(_red, 0.3)),
                ),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Text(title,
              style: const TextStyle(color: _textPrim, fontSize: 13, fontWeight: FontWeight.w700))),
          GestureDetector(
            onTap: onEdit,
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
              decoration: BoxDecoration(
                color: _o(accentColor, 0.12), borderRadius: BorderRadius.circular(8),
                border: Border.all(color: _o(accentColor, 0.3)),
              ),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(_IC.edit, color: accentColor, size: 12),
                const SizedBox(width: 4),
                Text('Edit', style: TextStyle(color: accentColor, fontSize: 11, fontWeight: FontWeight.w700)),
              ]),
            ),
          ),
        ]),
  ...
  Widget _profileInfoRow(IconData icon, String label, String value, {Color? valueColor}) =>
      Padding(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Flexible(
            child: Text(value,
                ...
                style: TextStyle(
                    color: valueColor ?? _textPrim, fontSize: 12, fontWeight: FontWeight.w600),
  ...
  Widget _profileStatBox(String label, String value, Color color) => Expanded(
        child: Container(
          ...
          child: Column(children: [
            ...
            const SizedBox(height: 4),
            Text(label,
                style: const TextStyle(color: _textSub, fontSize: 10, fontWeight: FontWeight.w600)),
          ]),
        ),
      );

  void _showEditDialog({required String title, required List<_EditField> fields}) {
    final controllers = {
        ...
        return Padding(
          ...
          child: SingleChildScrollView(
            child: Container(
              ...
              decoration: BoxDecoration(
                color: _navyLight, borderRadius: BorderRadius.circular(20),
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                Row(children: [
                  ...
                  const SizedBox(width: 8),
                  Text(title,
                      style: const TextStyle(color: _textPrim, fontSize: 16, fontWeight: FontWeight.w800)),
                  const Spacer(),
                  GestureDetector(
                ...
                ...fields.map((f) => Padding(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(f.label,
                        style: const TextStyle(color: _textSub, fontSize: 11, fontWeight: FontWeight.w600)),
                    const SizedBox(height: 6),
                    Container(
                      decoration: BoxDecoration(
                ...
                StatefulBuilder(builder: (ctx2, setSt) {
                  ...
                  return GestureDetector(
                    onTap: saving ? null : () async {
                      setSt(() => saving = true);
                      final updates = {for (final f in fields) f.key: controllers[f.key]!.text.trim()};
                      final ok = await _updateProfile(updates);
                      if (ctx2.mounted) Navigator.pop(ctx2);
                      _showSnack(ok ? 'Profile updated!' : 'Failed to save changes.', ok ? _green : _red);
                    },
                    child: Container(
                      width: double.infinity,
                      padding: const EdgeInsets.symmetric(vertical: 14),
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [BoxShadow(color: _o(_accent, 0.4), blurRadius: 10, offset: const Offset(0, 4))],
                      ),
                      child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
    ...
    ).whenComplete(() { for (final c in controllers.values) c.dispose(); });
  }
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                Row(children: [
                  ...
                  const Text('Change Password',
                      style: TextStyle(color: _textPrim, fontSize: 16, fontWeight: FontWeight.w800)),
                  const Spacer(),
                ...
                StatefulBuilder(builder: (ctx2, setSt) {
                  ...
                  return GestureDetector(
                    onTap: saving ? null : () async {
                      ...
                      if (ctx2.mounted) Navigator.pop(ctx2);
                      _showSnack(ok ? 'Password changed!' : 'Failed to change password.', ok ? _green : _red);
                    },
                    child: Container(
                      width: double.infinity,
                      padding: const EdgeInsets.symmetric(vertical: 14),
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [BoxShadow(color: _o(_purple, 0.4), blurRadius: 10, offset: const Offset(0, 4))],
                      ),
                      child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        Icon(_IC.save, color: Colors.white, size: 18),
                        SizedBox(width: 8),
