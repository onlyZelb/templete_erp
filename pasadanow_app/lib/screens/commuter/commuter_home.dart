...
class AppNotification {
  ...
  factory AppNotification.rideCompleted(String fare) => AppNotification(
        ...
        body: 'Your ride is done. You paid $fare. Thank you for riding PasadaNow!',
      );
            ...
            child: Column(
              children: [
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      const Icon(_IC.notification, color: _accent, size: 18),
                ...
                Expanded(
                  child: notifs.isEmpty
                          ...
                          child: Column(
                            ...
                            children: [
                              Icon(_IC.notification,
                                  color: _o(_textSub, 0.35), size: 44),
                              const SizedBox(height: 10),
                              const Text(
                                'No notifications yet',
                                style:
                                    TextStyle(color: _textSub, fontSize: 14),
                              ),
                              const SizedBox(height: 4),
                              const Text(
                                'Ride updates will appear here',
                                style:
                                    TextStyle(color: _textSub, fontSize: 11),
                              ),
                            ],
                          ),
                        )
                      : ListView.separated(
                          ...
                          itemBuilder: (_, i) =>
                              _NotifTile(notif: notifs[i], service: service),
                        ),
                ),
              ],
            ),
...
class _NotifTile extends StatelessWidget {
  ...
  const _NotifTile({required this.notif, required this.service});
  ...
  Widget build(BuildContext context) {
    ...
    return Dismissible(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: notif.read ? _card : _o(color, 0.07),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
            color: notif.read ? _cardBorder : _o(color, 0.35),
...
class _CommuterHomeState extends State<CommuterHome>
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: _o(_accent, 0.4), width: 1.5),
                  boxShadow: [
                    BoxShadow(
                      color: _o(_accent, 0.25),
                      blurRadius: 12,
                      spreadRadius: 0,
                    ),
                  ],
                ...
                child: ClipRRect(
                  ...
                  child: Image.asset(
                    ...
                    errorBuilder: (_, __, ___) =>
                        Icon(_IC.tricycle, color: _accent, size: 20),
                  ),
                ),
              ),
            ],
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    // Role pill — commuter specific styling
                    Container(
                      padding: const EdgeInsets.symmetric(
        ...
        GestureDetector(
          ...
          child: Padding(
            ...
            child: _AppBarBadge(
              ...
              topLabelColor: _fareConfig.surgeEnabled ? _orange : _accent,
              bottomLabel: '${_fareConfig.perKmRate.toStringAsFixed(0)}/km',
              bottomLabelColor: _fareConfig.surgeEnabled ? _orange : _textSub,
              borderColor: _fareConfig.surgeEnabled ? _orange : _accent,
  ...
  Widget _buildAppBarAvatar(AuthProvider auth) {
    ...
    return Container(
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        border: Border.all(color: _o(_accent, 0.55), width: 2),
        boxShadow: [
          BoxShadow(
            color: _o(_accent, 0.3),
            blurRadius: 10,
            spreadRadius: 0,
          ),
        ],
  ...
  Widget _buildBottomNav() => Container(
        decoration: BoxDecoration(
          gradient: const LinearGradient(
            ...
            colors: [Color(0xFF0D1E35), Color(0xFF081422)],
          ),
          border: Border(
            top: BorderSide(
  ...
  Widget _fareMetricsPanel() {
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Icon(_IC.priceChange, color: _accent, size: 16),
            const SizedBox(width: 8),
            ...
            if (cfg.surgeEnabled)
              Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(_IC.bolt, color: _orange, size: 10),
          ...
          Row(children: [
            _metricChip(_IC.distance, '${distKm.toStringAsFixed(2)} km',
                'Distance', _accent),
            const SizedBox(width: 8),
            _metricChip(_IC.time, '${_routeDurationMin!.toStringAsFixed(0)} min',
                'Est. Time', _purple),
            const SizedBox(width: 8),
          ...
          Text(
            ...
            style: const TextStyle(
                color: _textSub, fontSize: 9, fontStyle: FontStyle.italic),
          ),
        ]),
      ),
    );
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (_routeDistKm != null) ...[
          ...
          const SizedBox(height: 8),
          _fleetRow('Base Fare',
              '₱${_fareConfig.baseFare.toStringAsFixed(2)}', _textPrim),
          _fleetRow('Per-km Rate',
              '₱${_fareConfig.perKmRate.toStringAsFixed(2)}/km', _textPrim),
          _fleetRow('Minimum Fare',
              '₱${_fareConfig.minimumFare.toStringAsFixed(2)}', _textPrim),
          if (_fareConfig.bookingFee > 0)
            _fleetRow('Booking Fee',
                '₱${_fareConfig.bookingFee.toStringAsFixed(2)}', _textPrim),
...
// ─────────────────────────────────────────────────────────────────────────────
//  ★ SHARED APP BAR HELPER WIDGETS
// ─────────────────────────────────────────────────────────────────────────────

/// Encapsulated pulsing/static dot config for AppBarBadge
...
/// Reusable compact pill badge for the AppBar action area
...
/// Thin wrapper that gives the notification bell a consistent pill appearance
class _AppBarIconButton extends StatelessWidget {
  final Widget child;
  const _AppBarIconButton({required this.child});
