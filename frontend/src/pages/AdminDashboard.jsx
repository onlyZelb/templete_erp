...
async function callVerify(uid, role, newStatus, setUsers, setModalId) {
  try {
    const res = await fetch(`${DJANGO_URL}/api/admin/users/${uid}/verify`, {
      method: "PATCH",
...
// review button shared style helper
      ...
      >
        ...
        >
          {[
            ...
            ["Contact", user.phone || user.contact_no],
            ["Email", user.email],
            ["Joined", fmtD(user.created_at)],
          ].map(([l, v]) => (
            <div
...
function Dashboard() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${DJANGO_URL}/api/admin/users`);
    ...
    try {
      const res = await fetch(`${DJANGO_URL}/api/admin/trips`);
...
const s = {
  ...
  inputErr: { border: "1px solid rgba(248,113,113,.6)" },
  eyeBtn: {
    position: "absolute",
    right: 14,
    top: "50%",
