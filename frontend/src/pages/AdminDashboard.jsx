...
function isDriverOnline(d) {
  ...
  if (d.is_online === 1 || d.is_online === "1" || d.is_online === "true")
    return true;
  if (d.is_available === "1" || d.is_available === 1) return true;
  return false;
}
function getTripOrigin(t) {
  return t?.pickup_location || t?.origin || "—";
}
...
function OverviewView({ users, trips }) {
  ...
  const revenue = trips
    ...
    .reduce((a, t) => a + Number(t.fare || 0), 0);
  return (
    <>
      <div
        style={{
                ...
                <tbody>
                  {trips.slice(0, 5).map((t) => (
                    <tr key={t.id} style={ds.tr}>
                      ...
                      >
                        {getTripOrigin(t)} → {getTripDestination(t)}
                      </td>
                      <td
      ...
      >
        ...
        >
          {toImgSrc(user.profile_photo) ? (
            <img
              src={toImgSrc(user.profile_photo)}
              alt="Profile"
              onError={(e) => {
  ...
  return (
    <>
      ...
      >
        ...
        <StatCard
          icon="x"
          value={commuters.filter((c) => c.status === "rejected").length}
          label="Rejected"
          color="red"
        />
      </div>
      ...
      {modalUser && (
        <CommutterModal user={modalUser} onClose={() => setModalId(null)} />
      )}
    </>
  );
}
              ...
              <span
                style={{
                  ...
                  background: online
                    ? "rgba(74,222,128,.07)"
                    : "rgba(255,255,255,.04)",
                  color: online ? "#4ade80" : "#3a5a7a",
                  border: `1px solid ${online ? "rgba(74,222,128,.18)" : "rgba(60,110,160,.15)"}`,
                }}
              >
                <span
                  style={{
                    ...
                    height: 5,
                    borderRadius: "50%",
                    background: online ? "#4ade80" : "#3a5a7a",
                  }}
                />
  ...
  return (
    <>
      ...
      >
        ...
        <StatCard
          icon="grid"
          value={drivers.filter((d) => isDriverOnline(d)).length}
          label="Online Now"
          color="green"
        />
        <StatCard
